...
Distance gave **0.559 (0.476–0.641)** on the fitness screen, where 71 sites are affected, and **0.487 (0.421–0.554)** on the reporter screen, where 80 are. The two land on opposite sides of 0.5 from the same predictor on the same 1,475 sites. Both intervals contain 0.5, and the fitness value is the further from it: shuffling the outcome labels 20,000 times puts it 1.68 standard deviations out, at **p = 0.09**. That is not a small number and it is not significance either; it is reported as it stands.
...
|---|---:|---:|---:|
| Fitness screen, direction-corrected (declared) | 71 | 0.559 | 0.476–0.641 |
| Reporter screen, direction-corrected (declared) | 80 | 0.487 | 0.421–0.554 |
| Union of both screens, uncorrected (withdrawn) | 293 | 0.506 | 0.466–0.546 |
| Union, corrected for two directions and two screens | 85 | 0.541 | 0.464–0.617 |
| Top tenth by size of log fold change, fitness | 148 | 0.507 | 0.455–0.560 |
| Top tenth by size of log fold change, reporter | 148 | 0.509 | 0.457–0.561 |
| Top tenth by size of log fold change, larger of the two | 148 | 0.517 | 0.463–0.571 |
| The screens' own false-discovery control, below 0.25 | 19 | 0.614 | 0.433–0.781 |

Every interval contains 0.5. The two rows using the size of the fold change use no p-value at all, so they do not inherit the unresolved question about how the released columns were built.

#### 2.7.2 Two features that the pooled version hid

Keeping the screens apart makes two things visible that the union did not.

How deeply a residue is buried separates the classes in the fitness screen. Inverse relative solvent accessibility gives **0.604 (0.528–0.675)**, an interval that does not contain 0.5, against 0.524 (0.459–0.590) in the reporter screen. In the yeast cohort the same quantity had the largest comparator point estimate at 0.587 without excluding 0.5. Burial, not distance to an annotated residue, is the structural quantity that tracks this outcome.

In the reporter screen a feature that uses no structure at all beats the declared predictor. The smallest gap in sequence position to an eligible annotated residue gives 0.558 (0.492–0.624), and its paired difference against distance is **+0.071 (+0.005 to +0.137)**. That interval excludes zero, and it is the only paired difference in either cohort that does. Every other comparison in both screens contains zero.

#### 2.7.3 Three properties of the yeast measurement reappear

Restricting to targets carrying experimental evidence, which leaves 24 of 163 sites in yeast and so cannot be asked there, leaves **512 sites in 287 proteins** here: 0.575 (0.446–0.699) on the fitness screen and 0.491 (0.391–0.590) on the reporter screen. Annotation evidence does not move the reading in either.

The comparison remains almost entirely between proteins. Within-protein pairs are **102 of 99,684** on the fitness screen and **180 of 111,600** on the reporter screen, which is 0.10% and 0.16% against 2.65% in yeast. The larger cohort makes this worse, because adding proteins adds across-protein pairs quadratically and within-protein pairs linearly. Only 39 and 48 proteins contribute any within-protein comparison at all.
...
#### 2.7.4 A published feature separates the classes, in one screen
...
#### 2.7.5 What the two cohorts do not share
