...
def main() -> None:
    ...
    phase05 = json.loads((RESULTS / "robustness_statistics.json").read_text())
    parent_data = {
    ...
    child_data = {
        PRIMARY: pd.read_csv(RESULTS / "robustness_primary_analysis.csv"),
    ...
    checks: dict[str, bool] = {
        ...
        "named_cohort_contract_in_robustness_json": (
        ...
        "named_switch_consumed_by_robustness": all(
            token in (HERE / "src" / "02_robustness_analysis.py").read_text()
    ...
    for cohort, expected in canonical.items():
        ...
        checks.update({
            ...
            f"{cohort}_robustness_arm_table_matches_numbers": table_arm_matches(
    ...
    checks.update({
        ...
        "robustness_complete_arm_outputs": (
        ...
        "supplement_workbook_exists": (
            (RESULTS / "robustness_supplement.xlsx").is_file()
            and (RESULTS / "robustness_supplement.xlsx").stat().st_size > 10000
        ),
    })
    ...
    checks.update({
        ...
        "pae_pair_max_disclosed_in_robustness_narratives": all(
