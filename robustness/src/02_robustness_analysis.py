"""the robustness analysis robustness, continuous-outcome, and predictor-benchmark analyses."""
...
def main() -> None:
    all_data = pd.read_csv(RESULTS / "robustness_analysis.csv")
    ...
    d = primary_data
    primary_data.to_csv(RESULTS / "robustness_primary_analysis.csv", index=False)
    inclusive_data.to_csv(
        RESULTS / "robustness_inclusive_sensitivity_analysis.csv", index=False
    )
    ...
    d.to_csv(RESULTS / "robustness_analysis_with_oof_predictions.csv", index=False)
    publication_columns = [
    ...
    d[publication_columns].to_csv(RESULTS / "robustness_publication_data.csv", index=False)
    ...
    with (RESULTS / "robustness_statistics.json").open("w") as handle:
