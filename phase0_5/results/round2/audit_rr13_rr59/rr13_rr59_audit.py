...
RR-13 walks every stored interval in results/ and phase0_5/results/ (JSON + CSV) and records
nominal draws, retained draws, shortfall, and whether either endpoint touches exactly 0 or 1.
RR-59 reads phase0_5/results/predictor_benchmark.csv verbatim and re-derives the pooled
...
The frozen module phase0_5/src/02_phase0_5_analysis.py guards main() behind
...
def load_module():
    path = ROOT / "phase0_5/src/02_phase0_5_analysis.py"
    text = path.read_text()
...
def scan_csvs(rows):
    files = sorted(list((ROOT / "results").glob("*.csv")) +
                   list((ROOT / "phase0_5/results").glob("*.csv")))
    for f in files:
        with f.open() as fh:
...
def rr59(p05):
    import pandas as pd
    bench = pd.read_csv(ROOT / "phase0_5/results/predictor_benchmark.csv")
    oof = pd.read_csv(ROOT / "phase0_5/results/phase0_5_analysis_with_oof_predictions.csv")
...
def main():
    ...
    summary = {
        ...
        "notes": [
            "phase0_5/src/02_phase0_5_analysis.py bootstrap_auc returns draws=len(draws), the "
            "retained count after discarding single-class resamples. Every 'draws' value in "
            "phase0_5/results/ is therefore a genuine retained count.",
            ...
            "phase0_5/results/sift_comparator_sensitivity.csv omits a draws column, but the same "
            "intervals in phase0_5_statistics.json carry draws=20000.",
