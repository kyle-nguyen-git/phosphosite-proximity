"""Build the Phase 0.5 robustness summary figure (PDF and PNG)."""
...
def main() -> None:
    d = pd.read_csv(RESULTS / "phase0_5_primary_analysis.csv")
    ...
    statistics = json.loads((RESULTS / "phase0_5_statistics.json").read_text())
    ...
    for extension in ("png", "pdf"):
        for stem in ("phase0_5_robustness_summary", "phase0_5_two_arm_robustness_summary"):
            fig.savefig(
                RESULTS / f"{stem}.{extension}",
                dpi=300, bbox_inches="tight",
            )
    print("wrote phase0_5_robustness_summary.{png,pdf}")
